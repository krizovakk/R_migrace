...
sb_invc <- sbirky %>% 
  select(IdC_S, MINVC, Rada_S, PorC_S, Lomeni_S) # tabulka pro sledovani puvodniho a noveho inventarniho cisla
...
Prirustkova kniha nebyla vyplnena. Prirustkova cisla byla zadana do
Museionu rucne.
...
modify_tab <- merge(fond_skup, demus_tab, # fondskup <- previous chunk
...
if(file.exists(save)){
  ...
  write.table(interni_adresar, file = save, 
            sep = ";", quote = T, row.names = F, 
            fileEncoding="cp1250")
  print("Internal address book  S A V E D  !")
  }
...
**DEMUS**: Nazvy **MUSEION**: import_oznaceni.csv
...
**DEMUS**: Sbirky **MUSEION**: import_obp.csv
...
Import prvnich 10 zaznamu. Zkouska, zda sedi pocet a format sloupcu
apod.
...
**DEMUS**: Sbirky **MUSEION**: import_par.csv
...
**DEMUS**: Sbirky **MUSEION**: import_vum.csv
...
**DEMUS**: Sbirky **MUSEION**: import_fot.csv
...
**DEMUS**: Sbirky **MUSEION**: import_bio.csv
...
**DEMUS**: Sbirky **MUSEION**: import_br.csv
...
Pro doprovodne tabulky je treba mit jiz pripravene inventarni cislo v
konecnem formatu\
(viz sekce *Referencni tabulka Sbirky*)

## Materál předmětu

tabulka s poradim materialu ke konkretnimu predmetu invc1 - 1. material
1 invc1 - 2. material 2 invc2 - 1. material 1, atd
...
modif_save <- paste0(path_csv, spravce, "_modif_matPtu.csv")
...
save <- paste0(path_csv, spravce, "_import_matPtu.csv")
...
tabulka s poradim technik ke konkretnimu predmetu invc1 - 1. technika 1
invc1 - 2. technika 2 invc2 - 1. technika 1, atd

**DEMUS**: Sbirky **MUSEION**: import_techPtu.csv
...
modif_save <- paste0(path_csv, spravce, "_modif_techPtu.csv")
...
save <- paste0(path_csv, spravce, "_import_techPtu.csv")
...
save <- paste0(path_csv, spravce, "_import_rozmPtu.csv")
...
tabulka s trojici ke konkretnimu predmetu invc1 - 1. autor invc1 - 2.
datace invc1 - 3. misto vzniku invc1 - 4. autor 2 invc2 - 1. datace, atd
...
**DEMUS**: Sbirky **MUSEION**: import_vznikPtu.csv
...
interni_adresar <- read.table(paste0(path_csv, "_", spravce, "_INTERNI_ADRESAR.csv"),
                        header = T, sep = ";", fileEncoding="cp1250")
...
load <- read_excel(paste0(path_data, "Manipulace.xlsx"))

dupli_entr <- load %>% # kontrola duplicit
  group_by(IdC_MN, DatMan_MN) %>% 
  filter(n() != 1)

demus_tab <- load %>% 
  filter(!is.na(Stav_MN)|!is.na(PopisSt_MN)) %>% 
  group_by(IdC_MN) %>% 
  mutate(dupli = ifelse((duplicated(IdC_MN, fromLast=TRUE)), "1", "0")) %>% 
  filter(!dupli == "1")
...
modify_tab <- merge(sb, demus_tab, by.x = "IdC_S", by.y = "IdC_MN", 
   ...
   mutate(MEMPTY = "",
          ...
          '2sbirkaCisloEvidInt' = sbirka,  
          '3podsbirkaCislo' = podsbirka, 
          '9datum' = format(as.Date(DatMan_MN),"%d.%m.%Y"),
          '10zapsal' = "import", 
          '11nalezen' = "1")
...
**IMPORTNÍ ROZHRANNÍ**: CSV Uložení předmětu

-   Stala Lokace a Aktualni Lokace -> porovnat
-   Pokud je to stejne, Aktualni lokace se nemigruje
-   Pokud nejsou stejne, Stala = Stalam, Aktualni = Docasna

```{r TABLE ULOZENI PTU FROM TAB SBIRKY}

# L O A D

dopr_tab <- sbirky %>% 
  select(StLokace_S, AktLokace_S, MINVC)
dopr_tab[is.na(dopr_tab)] <- ""

lok_check <- dopr_tab %>% 
  filter(StLokace_S != AktLokace_S) # 63x St = NA, Akt != NA; 1x St != Akt
...
modify_tab <- dopr_tab %>% 
  mutate(MEMPTY = "",
         MDEPOZITAR = "H11",
         MSTALA = ifelse(StLokace_S == "", NA, paste0(MDEPOZITAR, "-", StLokace_S)), 
         MAKTUAL = ifelse(StLokace_S == AktLokace_S, "", paste0(MDEPOZITAR, "-", AktLokace_S)),
         '1typKarty' = "K")

modif_save <- paste0(path_csv, spravce, "_modif_ulozeniPtu.csv")
...
save <- paste0(path_csv, spravce, "_import_ulozeniPtu.csv")
...
### slovnik ulozeni predmetu

**DEMUS**: Sbirky
**MUSEION**: import_ulozeni.csv

**IMPORTNÍ ROZHRANNÍ**: CSV Uložení
...
modify_tab <- oper_ulozeniPtu %>% 
  select('7staleUlozeniKod') %>% 
  mutate(MEMPTY = "", 
         MDEPOZITAR = "H11") %>% 
  distinct() %>% 
  drop_na('7staleUlozeniKod')

modif_save <- paste0(path_csv, spravce, "_modif_ulozeni.csv")
...
mus_tab <- modify_tab %>% 
  select(
    '1depozitarKod' = MDEPOZITAR,  # !! povinne !!
    '2ulozeniKod'= '7staleUlozeniKod',           # !! povinne !!
    '3lokaceKod' = MEMPTY
    )

oper_ulozeni <- mus_tab

# S A V E   I M P O R T   C S V

save <- paste0(path_csv, spravce, "_import_ulozeni.csv")
...
vypis <- read.table(paste0(path, "data/MDH/KD/vypis.csv"), 
                    sep = ";", header = T, fileEncoding="cp1250") # split path in Excel

# S B I R K Y

invc <- sb_invc %>% 
  mutate(MVYPINVC = ifelse(Lomeni_S == "_", paste0(Rada_S, "_", PorC_S), paste0(Rada_S, "_", PorC_S, "_", Lomeni_S)),
         PorC_S = as.character(PorC_S))

# R E L A T E 

# H11T a H11U se lisi formatem nazvu souboru, proto se budou resit zvlast

## H 11 T

h11t <- vypis %>% 
  filter(!str_detect(file, "^pohledy.*")) %>% 
  filter(str_detect(file, "^H11T.*")) %>% 
  separate(file,into=c("rada", "porcis"), sep = "-", 
            extra = "merge", remove = FALSE) %>% 
  separate(porcis,into=c("por", "podl"), sep = "_", 
            extra = "merge", remove = FALSE) %>%
  mutate(por = ifelse(str_detect(por, ".*a|b$"), str_sub(por,1,nchar(por)-1), por),
         por = str_trim(por))
...
h11u <- vypis %>% 
  filter(!str_detect(file, "^pohledy.*")) %>% 
  filter(str_detect(file, "^H11U.*")) %>% 
  separate(file,into=c("rada", "porcis"), sep = "_", 
            extra = "merge", remove = FALSE) %>% 
  separate(porcis,into=c("por", "poradi"), sep = "_", 
            extra = "merge", remove = FALSE) %>% 
  mutate(podl = ifelse(str_detect(por, ".*a|b$"), 
                       # separate(por,into=c("por", "poradi"), sep = "_", extra = "merge", remove = FALSE) %>% 
                       str_sub(por,-1, -1), "-"))


# M O D I F Y

# modify_tab <- left_join(h11t, invc, by = c("rada" = "Rada_S", "por" = "PorC_S"), keep = T) %>%      # H 11 T     # s w i t c h
modify_tab <- left_join(h11u, invc, by = c("rada" = "Rada_S", "por" = "PorC_S"), keep = T) %>%    # H 11 U     # s w i t c h
  select(MINVC, Rada_S, rada, PorC_S, por, file, ext) %>% 
  group_by(rada, por) %>% 
  mutate(MPORADI = seq_along(file)) %>% 
  ungroup() %>%
  mutate(MEMPTY = "",
         '1uloha' = "PREDMET_PRIVATE",
         '6url' = paste0(file, ".", ext),
...
# modif_save <- paste0(path_csv, spravce, "_modif_obrazky_h11t.csv")       # s w i t c h
modif_save <- paste0(path_csv, spravce, "_modif_obrazky_h11u.csv")     # s w i t c h
...
# oper_obrazky_h11t <- mus_tab   # s w i t c h 
oper_obrazky_h11u <- mus_tab   # s w i t c h

# S A V E   I M P O R T   C S V

# save <- paste0(path_csv, spravce, "_import_obrazky_h11t.csv")      # s w i t c h 
save <- paste0(path_csv, spravce, "_import_obrazky_h11u.csv")    # s w i t c h
...
nesparovane <- vypis %>% 
  filter(!str_detect(file, "^H11U.*|^H11T.*")) %>% 
  select(path) %>% 
  write.table(file = paste0(path, "data/MDH/KD/vypis_nesparovane.csv"), 
...
```{r}
imp <- read.table("C:/Users/krizova/Documents/R/mus_csv/NM-HM/MDH/NM-HM_import_obp.csv", 
                  header = T, sep = ";", fill = T, fileEncoding="cp1250", dec = ",")
```
