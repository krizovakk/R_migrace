...
### Vytvoreni inventarniho cisla INVC
...
write.table(mus_tab, paste0(path_csv, project, "_csv_fond.csv"), 
...
write.table(mus_tab, paste0(path_csv, project, "_csv_skupiny.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_adresar.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_oo.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250") 
...
write.table(mus_tab, paste0(path_csv, project, "_csv_po.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
# write.table(mus_tab, paste0(path_csv, project, "_csv_lokalita.csv"), 
#             sep = ";", quote = F, row.names = F, 
#             na = "", fileEncoding="cp1250")
...
```{r LOAD AND MODIFY TABLE MATERIALY, warning = F}
...
write.table(mus_tab, paste0(path_csv, project, "_csv_material.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
```{r LOAD AND MODIFY TABLE TECHNIKY, warning = F}
...
write.table(mus_tab, paste0(path_csv, project, "_csv_technika.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
**DEMUS**: Nazvy 
**MUSEION**: csv_oznaceni.csv

**IMPORTNÍ ROZHRANNÍ**: CSV Označení 

```{r LOAD AND MODIFY TABLE NAZVY, warning = F}
...
write.table(mus_tab, paste0(path_csv, project, "_csv_oznaceni.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
**DEMUS**: Sbirky 
**MUSEION**: csv_dataceNazvem.csv

**IMPORTNÍ ROZHRANNÍ**: CSV Datace názvem

```{r EXTRACT DATATION FROM TABLE SBIRKY, warning = F}

modify_tab <- sbirky

```

```{r CREATE CSV DATACE NAZVEM}
...
#     '5oznaceniPoznamka' = Pozn_NZ)
# 
# write.table(mus_tab, paste0(path_csv, project, "_csv_dataceNazvem.csv"), 
#             sep = ";", quote = F, row.names = F, 
#             na = "", fileEncoding="cp1250")
# 
# oper_datace <- mus_tab
...
SELECT "" As 1sbirkaCisloEvidInt, "" As 2podsbirkaCislo, "" As 3dataceNazvemNazev, "" As 4dataceHodnota, "" As 5dataceSnad, "" As 6kvalifikatorDataceNazev, "" As 7dataceSnad2, "" As 8kvalifikatorDatace2Nazev, "" As 9dataceNazvemPoznamka, "" As 10dataceNazvemPublicNadNazev, "" As 11dataceNazvemPublicNaz FROM TABULKA;


\newpage

# 3. DOPROVODNÉ TABULKY (opakovatelné)
...
write.table(mus_tab, paste0(path_csv, project, "_csv_matpred.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_techpred.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
invc1 - 1. autor
invc1 - 1. datace
invc1 - 1. misto vzniku
invc1 - 2. autor 2
invc2 - 2. datace 2, atd  
...
```{r CHECK ROLES AND LOCS DICTIONARIES}
...
write.table(mvzn_tab, paste0(path_csv, project, "_csv_lokalita_MVZN.csv"),
            sep = ";", quote = F, row.names = F,
            na = "", fileEncoding="cp1250")
...
**DEMUS**: Sbirky 
**MUSEION**: csv_vznikPtu.csv  
  
**IMPORTNÍ ROZHRANNÍ**: CSV Vznik předmětů  


```{r DERIVE VZNIKPTU FROM SBIRKY, warning = F}

dopr_tab <- sbirky %>% 
  select(MINVC, Autor_S, Role_S, Datace_S, MistoVz_S) %>% 
  mutate(MEMPTY = "") %>% 
  mutate(MDATACE = Datace_S) %>% 
  mutate(MDAT_Y = ifelse(str_detect(MDATACE, "_00_00"), MDATACE, "")) %>% 
  mutate(MDAT_Y = str_replace_all(MDAT_Y, "_00|-00", ""))

ym <- dopr_tab %>% 
  # mutate(MDAT_YM = ifelse(str_detect(MDATACE, "/^[^_]*_[^_]*_*00$/"), MDATACE, "")) 
  # mutate(MDAT_YM = ifelse(str_detect(MDATACE, "\\w*_*[^0]*[^0]*_*\\w"), MDATACE, "")) 
  mutate(MDAT_YM = ifelse(str_detect(MDATACE, "\\.(00_00)$"), "", MDATACE)) 


  mutate(MDATACE = str_replace_all(dopr_tab$MDATACE, "-00", "")) %>% 
  mutate(MDAT_LATER = ifelse(str_detect(dopr_tab$MDATACE, "XX"), dopr_tab$MDATACE, "")) %>% 
  mutate('2poradi' = "") %>% 
  mutate('15dataceSnad' = ifelse(str_detect(dopr_tab$MDATACE, "\\?"), 1, 0))


  
```

```{r CREATE CSV VZNIKPTU}

mus_tab <- dopr_tab %>% 
  select(
    '1SPCislo' = MINVC,                          # !! povinne !!
    '2poradi' = '2poradi',                       # !! povinne !!
    '3autorSubjektKod' = Autor_S,             
    ...
    '11dataceHodnota' = Datace_S, 
    '12datacePoznamka' = MEMPTY, 
    '13poznamka' = MEMPTY, 
    '14kvalifikatorDatace' = MEMPTY, 
    '15dataceSnad' = MEMPTY,                     # priznak 0 1 
    '16kvalifikatorDatace2' = MEMPTY,  
    '17dataceSnad' = MEMPTY)                     # priznak 0 1  

write.table(mus_tab, paste0(path_csv, project, "_csv_vznikPtu.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_obp.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab_cvic, paste0(path_csv, project, "_csv_obp_cv.csv"),
            sep = ";", quote = F, row.names = F,
            na = "", fileEncoding="cp1250")
```
...
write.table(mus_tab, paste0(path_csv, project, "_csv_par.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
```
...
write.table(mus_tab, paste0(path_csv, project, "_csv_vum.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_fot.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_bio.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
...
write.table(mus_tab, paste0(path_csv, project, "_csv_br.csv"), 
            sep = ";", quote = F, row.names = F, 
            na = "", fileEncoding="cp1250")
```
