...
dat <- jednot %>% 
  mutate(Mdat_ser = as.numeric(ifelse(str_detect(datumNabyti, "^.....$"), datumNabyti, NA)),
         ...
         Mdat = case_when(str_detect(datumNabyti, "^.....$") ~ Mdat_trans,
                          ...
                          TRUE ~ datumNabyti)) %>%
         # Mdat = ifelse(str_detect(datumNabyti, "^.....$"), Mdat_trans, 
...
aa <- dat %>% 
  ...
  mutate(evid15 = "1") %>% relocate(evid15, .after = pocetks13) %>% 
...
bb <- aa %>% # datum nabyti reseno zvlast o sekci vyse
...
cc <- bb %>% 
  ...
  mutate(pozn20 = ifelse(is.na(pocetks13), paste("pocetKs:", pocetKs), NA),
         nabpozn29 = ifelse(is.na(datumNabyti)&is.na(zpNabyti), "", # jinak se mi do prazdnych vkladalo pres paste tvrde NA
  ...
  relocate(nabpozn29, .after = datnab22) %>% 
  select(-is_valid_date, -is_valid_date2, -Mdat)
...
# inventarni cisla pro single hodnoty 
...
val_sign <- c("A", "B", "D", "F", "Fi", "Fo", "Fo", "G" ,"GK", "H", "K", "LA", "M" ,"ME", "Mi", "MP",
              "N", "O", "P", "PM", "PP", "R", "S","ST", "UP", "V", "VS", "ZE", "ZV")
...
inv <- cc %>% 
  ...
  mutate(puv_invc106 = invc,
         ...
         Minvc0 = case_when(Sign == "A" ~ paste0(Sign, str_pad(Minvc, 5, pad = "0")),
...
type <- ee %>% 
  ...
  drop_na(invc) %>% 
  mutate(ciste_intervaly = str_extract())
...
res <- mer %>% 
  ...
  select(-typ_cisla, -numbers, -Minvc, -Minvc0, -Msub, -Mprirc)
