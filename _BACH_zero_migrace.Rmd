...
biolib <- read.csv(paste0(path, "bioslov/BioLib_KKsummary_plantae.csv"), sep = ";")  # plantae obsahuje i mechy
...
# lokalita, geo    
    ...
    '44dataceHodnota' = sber_d, # <- datum sberu !!!!!
    '45datacePoznamka' = X, 
# etikety
...
dats <- bach %>% select(MINVC, sber_d) %>% 
  filter(!is.na(sber_d)|sber_d!="") %>% 
  mutate(status = case_when(str_detect(sber_d, "\\d+\\.\\d+\\.\\d\\d\\d\\d") ~ "datum", TRUE ~ "jiny format")) %>% 
  filter(status == "jiny format")
  # filter(status == "datum")


write.xlsx(dats, paste0(path_proj, "diskuse/MUZTE_botanika_dataceSberu.xlsx"), row.names = F)
write.xlsx(dats, paste0(path_proj, "diskuse/MUZTE_botanika_dataceSberu_spatnyformat.xlsx"), row.names = F)
write.table(dats, paste0(path_proj, "diskuse/MUZTE_botanika_dataceSberu.csv"), 
            quote = F, row.names = F,
            sep = ";", dec = ",",
            na = "", fileEncoding="cp1250")
...
check <- full_join(dop_sber1, dop_sber2, by = c("MINVC" = "MINVC"))
...
*IMPORTNÍ ROZHRANÍ*: CSV Ulozeni Predmetu
*MUSEION*: import_ulozPtu.csv  
...
dop_lok <- bach %>% 
  select(X, MINVC, lokace) %>% 
  mutate(typkarty1 = "K",
         stale7 = case_when(lokace == "" ~ "",
                            TRUE ~ "depozitář botaniky"),
         pozn10 = ifelse(str_detect(lokace, "zapůjčeno Olomouc"), "zapůjčeno Olomouc", ""))
