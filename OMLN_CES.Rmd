...
check2 <- pk_sparovano %>% filter(CES_cislo != Mnove_cisloCES)
...
CES obsahuje subcisla za lomitkem 'A 482/1

MCES_par_a = Mrada, Mporc, "/" , Msubc
MCES_par_b = Mrada, Mporc --------------------------> nakonec vyhodnoceno jako blbost, žejo. CES cislo menit nechci. 
...
rel_a <- left_join(CES_ka %>% filter(zmena != "R"), MUS_ka, by = c("MCES_par_a" = "MMUS_par_a"), keep = T) %>% 
  group_by(MMUS_par_a) %>% 
  mutate(is_podl = ifelse(n() == 1, F, T)) %>% 
  relocate(is_podl, .before = MMUS_par_a)


REL_ka_NV <- rel_a %>% 
  mutate(status = case_when(!is.na(MMUS_par_a)&!is.na(MCES_par_a) ~ "sparovano", 
                             is.na(MMUS_par_a) ~ "nenalezeno v MUSEIONu",
                            is.na(MCES_par_a) ~ "nenalezeno v CES"),
         REL_zdroj = ifelse(!is.na(CES_podsb), CES_podsb, "TBM")) %>% 
         # REL_podsb = coalesce(podsbirka, CES_podsb)) %>%
  select("MUS_id" = sp_id, "MUS_invc" = predmet_cislo, "MUS_cisloCES" = sp_cisloCES, "CES_cislo" = cislo, "MUS_podsb" = podsbirka, CES_podsb, REL_zdroj,
         MMUS_par_a, MCES_par_a, MMUS_par_b, MCES_par_b, zmena, duvod, typ_cisla, status) %>% 
  group_by(MUS_id) %>% 
  mutate(is_REL_dupl = ifelse(n() == 1, "OK", 
                              ifelse(is.na(MMUS_par_a), NA, "DUPL"))) %>% 
  ungroup()

# Warning: Detected an unexpected many-to-many relationship between `x` and `y`. ----> pridala jsem vyse check na duplicity

REL_ka_NV$status <- as.factor(REL_ka_NV$status)
summary(REL_ka_NV$status)

# ------------------- b

rel_b <- left_join(REL_ka_NV %>%
                     filter(status != "sparovano") %>% 
                     select(MUS_id, MUS_invc, MUS_cisloCES, MUS_podsb, MMUS_par_b, REL_zdroj, status), 
                   CES_ka %>% 
                     filter(zmena != "R"), 
                   by = c("MMUS_par_b" = "MCES_par_b"), keep = T) 
...
A - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
DS - CES cislo 'Ds ##', v MUS 'DS' ...
G - jsou hlášena jednoslivá čísla pro hromadnou kartu
H - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
Hf - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
L - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
Nm -  -||-
NP ´CES cislo 'Np ##', v MUS 'NP', +  udelat druhe kolo parovani bez subcisla
S - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
Z - nektera cisla jsou podlomena -> udelat druhe kolo parovani bez subcisla
...
rel1 <- left_join(CES_ka, MUS_ka , by = c("MCES_par_a" = "MMUS_par_a"), keep = T) %>% 
  mutate(status = ifelse(!is.na(MCES_par_a)&!is.na(MMUS_par_a), "sparovano", "ne"),
         rel = "CES podl MUS podl")

rel1$status <- as.factor(rel1$status)
summary(rel1$status)                          #  8289     23169 
spa1 <- rel1 %>% filter(status == "sparovano")
nespa1 <- rel1 %>% filter(status == "ne")



rel2 <- left_join(rel1 %>% filter(status == "ne") %>% 
                    select(-MMUS_par_a, -MMUS_par_b, -MUS_cislo, -MUS_id, -MUS_cisloCES, -status), 
                  MUS_ka, by = c("MCES_par_a" = "MMUS_par_b"), keep = T) %>% 
  mutate(status = ifelse(!is.na(MCES_par_a)&!is.na(MMUS_par_b), "sparovano", "ne"),
         rel = "CES podl MUS nepodl")

rel2$status <- as.factor(rel2$status)
summary(rel2$status)  
spa2 <- rel2 %>% filter(status == "sparovano")



rel3 <- left_join(rel2 %>% filter(status == "ne") %>% 
                    select(-MMUS_par_a, -MMUS_par_b, -MUS_cislo, -MUS_id, -MUS_cisloCES, -status), 
                  MUS_ka, by = c("MCES_par_b" = "MMUS_par_a"), keep = T) %>% 
  mutate(status = ifelse(!is.na(MCES_par_b)&!is.na(MMUS_par_a), "sparovano", "ne"),
         rel = "CES nepodl MUS podl")

rel3$status <- as.factor(rel3$status)
summary(rel3$status)  
spa3 <- rel3 %>% filter(status == "sparovano")



rel4 <- left_join(rel3 %>% filter(status == "ne") %>% 
                    select(-MMUS_par_a, -MMUS_par_b, -MUS_cislo, -MUS_id, -MUS_cisloCES, -status), 
                  MUS_ka, by = c("MCES_par_b" = "MMUS_par_b"), keep = T) %>% 
  mutate(status = ifelse(!is.na(MCES_par_b)&!is.na(MMUS_par_b), "sparovano", "ne"),
         rel = "CES nepodl MUS nepodl")

rel4$status <- as.factor(rel4$status)
summary(rel4$status)  
spa4 <- rel4 %>% filter(status == "sparovano")

nrow(spa1) + nrow(spa2) + nrow(spa3) + nrow(spa4)
nrow(spa1) + nrow(spa2) + nrow(spa3) 
