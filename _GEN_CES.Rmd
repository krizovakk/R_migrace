...
ces_pr <- pr %>% 
  mutate(Mrada = ifelse(str_detect(cislo, "[:alpha:]"), gsub("[^a-zA-Z-]+", "", cislo), NA),
         ...
         # Msub = ifelse(str_detect(cislo, "\\/"), gsub(".*\\/", "", cislo), NA),
         # Msub = ifelse(str_detect(Msub, "20\\d\\d"), gsub(".*", NA, Msub), Msub),
         # Msub = ifelse(str_detect(Msub, "\\d\\d"), paste0("19", Msub), Msub),
         # Mcislo = gsub("\\/.*", "", Mcislo),
         ) %>% 
  mutate(Mrada = case_when(Mrada == "ZMK" ~ "K"), TRUE ~ Mrada) %>% # uprava kodu rady podle pozorovanych pripadu
  # mutate(Mrok = ifelse(str_detect(cislo, "\\d+\\/20\\d\\d"), sub(".*/", "", cislo), NA),
  #        Mrada = ifelse(str_detect(cislo, "[:alpha:]"), gsub("[^a-zA-Z-]+", "", cislo), NA),
  #        Mcislo = gsub("[^0-9/]+", "", cislo),
  #        Msub = ifelse(str_detect(cislo, "\\/"), gsub(".*\\/", "", cislo), NA),
  #        Msub = ifelse(str_detect(Msub, "20\\d\\d"), gsub(".*", NA, Msub), Msub),
  #        Msub = ifelse(str_detect(Msub, "\\d\\d"), paste0("19", Msub), Msub),
  #        Mcislo = gsub("\\/.*", "", Mcislo),
  ...
  select(zdroj, typ_cisla, zmena, duvod, cislo, MCES_par)
  
dupl <- ces_pr %>% group_by(cislo) %>% filter(n() != 1) %>% 
...
dupl <- ces_pr %>% group_by(MPRIRC) %>% filter(n() != 1) %>% 
  mutate(popis_chyby = "duplicitni cislo CES")  
...
# -> uprava kodu rady, tvorba parovaciho cisla 
...
# r a d a    
    Mrada = case_when(
      ...
      str_detect(Mc, "^L\\d+") ~ "?", # je v CES, ale v MUS ne !!!!!!!!!!!!!!!!!
...
# check <- ces_ic %>% select(Mpattern, Mrada) %>% 
#   mutate(check = gsub("[^[:alpha:]]+", "", Mrada)) %>% 
#   filter(Mpattern != check) %>% distinct()
...
pair_not_MUS <- left_join(pair_not_MUS0, mus_ic, by = c("MCES_par2" = "MMUS_par"), keep = T) %>% 
  mutate(status = case_when(is.na(MMUS_par) ~ "nenalezeno v MUSEIONU",
                            TRUE ~ "nesrovnalosti v podlomení"),
         akce = ifelse(is.na(MMUS_par), case_when(zmena == "N" ~ " -> nutno dohledat vazbu na MUSEION",
                          zmena == "V" & duvod == "N" ~ "",
                          zmena == "V" & duvod == "P" ~ "",
                          zmena == "R" & duvod == "C" ~ "",
                          zmena == "R" & duvod == "Z" ~ "",
...
# plyr::count(SAVE3[,c('zmena','duvod')])
...
pair_not_CES0 <- rel0 %>% filter(is.na(MCES_par)) %>% 
  select(MMUS_par, MMUS_par2, predmet_cislo, sp_cislo, sp_id) 

pair_not_CES <- right_join(ces_ic, pair_not_CES0, by = c("MCES_par" = "MMUS_par2"), keep = T) %>% 
...
### Report CES --- workspace ---

```{r}

# table1 
s1 <- sort(unique(ces$zdroj))
s2 <- sort(unique(mus_ic$nazev))

zdroje <- data.frame(s1)

write.xlsx(zdroje, paste(path_ces, proj, "_zdroje.xlsx"), row.names=F, showNA = F)
write.xlsx(as.data.frame(s2), paste(path_ces, proj, "_zdroje.xlsx"), sheetName = "dva", append = T, row.names=F, showNA = F)

ic$zdroj <- factor(ic$zdroj)
summary(ic$zdroj)
...
summary(ces_ic$zmena)
summary(ces_ic$duvod)
...
```{r}
pc <- ces_ic %>% 
  filter(Mrada %in% c("Pc", "PcCv")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

pck <- ces_ic %>% 
  filter(Mrada %in% c("Pc-K")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

pl <- ces_ic %>% 
  filter(Mrada %in% c("Pl", "PlCV")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

s <- ces_ic %>% 
  filter(Mrada %in% c("S", "SCV")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

tx <- ces_ic %>% 
  filter(Mrada %in% c("Tx", "TxCV")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

zm <- ces_ic %>% 
  filter(Mrada %in% c("ZM", "ZMCV")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))

cv <- ces_ic %>% 
  filter(Mrada %in% c("CV-N", "CV - N")) %>% 
  mutate(Mcislo_0 = as.numeric(Mcislo_0))
