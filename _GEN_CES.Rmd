...
# ces <- read.csv(paste0(path_ces, proj, "/OMCV_allTXT.csv"),
#                 header = T, fill =T, sep = ";",
#                 fileEncoding="cp1250")
...
relz <- full_join(icz, mus_icz, by = c("MCES_par" = "MMUS_par2"), keep = T) %>% 
  select(zdroj, zdroj_kod, zmena, duvod, CES_cislo, MCES_par,
         MMUS_par, MMUS_par2, MUS_cislo = predmet_cislo, MUS_rada = rada, sp_id, sp_cisloCES, podsb, nazev)
...
icz_spar <- relz %>% filter(!is.na(MCES_par)&!is.na(MMUS_par2))
icz_mus0 <- relz %>% filter(is.na(MMUS_par2)) %>% arrange(MCES_par)
...
# clear <- ces_pr %>% group_by(MCES_par) %>% filter(n() == 1)
