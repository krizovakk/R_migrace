...
nejprve pro jeden soubor, pote udelat smycku podle pole 'zdroj' ?

#### 01 Archeologicka n=163
...
pk_n <- CORE_ces %>% 
  ...
  # !!! toto se mezi podsbirkami lisi -> URPAVIT !!!  vvvvvvvvvvv 
  
  mutate(
         Mcislo = ifelse(str_detect(cislo, "\\/"), gsub("\\/.*", "", cislo), cislo),
         Mcislo0 = str_pad(Mcislo, 4, pad = "0"),
         Mrok = ifelse(str_detect(cislo, "\\/"), gsub(".*\\/", "", cislo), NA),
         Mrok19 = ifelse(str_detect(Mrok, "^\\d\\d$"), paste0("19", Mrok), Mrok),
         MCES_par = paste0(Mcislo0, "/", Mrok19)) %>% 
  
  # !!! toto se mezi podsbirkami lisi -> URPAVIT !!!  ^^^^^^^^^^^^
...
#### 03 Archeologická-fond_Archeologického_ústavu n=19
...
rem <- CORE_ces %>% 
  filter(zdroj == podsb_ces) # kontrola poctu zaznamu, protoze N = 1
...
rem <- CORE_ces %>% 
  filter(zdroj == podsb_ces) # kontrola poctu zaznamu, protoze N = 0
...
#### D 17 Historicka n=4589

N 3818 -> p 3390 m 431
VR 771

!!! D =  duplicity v rel přes MMUS_par !!!
...
#### D 18 Historicky dokument n=2152
...
#### 19 Muzeum Rumburk n=10592

N 10263 -> p 3454 m 6809
VR 329
