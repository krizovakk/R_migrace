...
*!!! NIZE JEN OVERENE A FUNKCNI PRIKAZY pro XML!!!*    
...
```{r TRIAL} 
# path_data <- "C:/Users/katerina.krizova/Documents/R/piskoviste/marc21/"
# single
# df <- read.table(paste0(path_data, "export-MAD-AudiovizualniDokumenty-240528-17zaznamu.txt"), 
#                  fileEncoding="cp1250", header = F, fill = T)
# tags <- df %>% 
#   select(V1) %>% 
#   filter(str_detect(V1, "^[0-9]+$")) %>% 
#   distinct() %>% 
#   mutate(num_tag = as.numeric(V1)) %>% 
#   arrange(num_tag)

# Lmarc <- list.files(path_data, pattern = "*.txt")  # *.txt *.jpg ...
# 
# fill <- data.frame()
# # fill <- data.frame(matrix(ncol = 1, nrow = 0))
# 
# tags <- for(i in Lmarc) {
#   a <- read.table(paste0(path_data, "export-MAD-HudebninyRukopisy-240528-40zaznamu.txt"), 
#                   fileEncoding="cp1250", header = F, fill = T)
#   b <- a %>% 
#     select(V1) %>% 
#     filter(str_detect(V1, "^[0-9]+$"))
...
#   arrange(num_tag)
# read lines
# path_data <- "C:/Users/katerina.krizova/Documents/R/piskoviste/marc21/export-MAD-AudiovizualniDokumenty-240528-17zaznamu.txt"
# lines <- readLines(path_data)
# tags <- substr(lines, 1,4)
...
# path_data <- "C:/Users/katerina.krizova/Documents/R/piskoviste/marc21/MAD/"
# path_data <- "C:/Users/katerina.krizova/Documents/R/piskoviste/marc21/MBS/"
path_data <- "C:/Users/katerina.krizova/Documents/R/piskoviste/marc21/CMH1/"
...
for(i in Lmarc) {
  a <- readLines(paste0(path_data, i))
  b <- substr(a, 1,4)
  c <- data.frame(b)
  # c <- data.frame(a)
  fill <- rbind(fill, c)
...
Rev_data <- read_excel("M:/03 klienti/narodni galerie - NG/ng konverze/2024-CKS-plastiky+Nm/revize-KK/proKK_revize.xlsx", col_types="text")
Sb_stav <- read_excel("M:/03 klienti/narodni galerie - NG/ng konverze/2024-CKS-plastiky+Nm/revize-KK/proKK_stav.xlsx", col_types="text")
...
fin <- closedate %>% 
  ...
  # distinct(sb_data_id, MDATE_rev, .keep_all = TRUE) %>% 
  select(sb_data_id, rev_data_id, "datum_rev" = MDATE_rev, "datum_stav" = MDATE_stav, "MSTAV" = kk_MSTAV, "MPOPISSTAVU" = kk_MPOPISSTAVU) %>% 
  mutate(MPOPISSTAVU = str_trim(MPOPISSTAVU))
  
check <- fin %>% group_by(sb_data_id) %>% filter(n()>4)  

write.table(fin, file = "M:/03 klienti/narodni galerie - NG/ng konverze/2024-CKS-plastiky+Nm/revize-KK/KK_revize_stavy_n12340_TRIM.csv", 
            quote = F, row.names = F, 
            sep = ";", dec = ",", 
            na = "", fileEncoding="cp1250") # quote = F proto, ze v datech jsou uvozovky a v prvnim pripade pri ulozeni pridava backslash
...
filepath = jmeno souboru i se zarazenim do ciselnych podslozek
filename = pouze jmeno souboru (muze byt duplicitni)

## csv

sbirka::invc = image::ident_id
tabulka image obsahuje cestu k souborum ve sloupci 'data_1'

```{r}
path_ng <- "M:/03 klienti/narodni galerie - NG/ng konverze/2024-CKS-plastiky+Nm/KK_obrazky/"

path_img <- "D:/Fotoarchiv NG/"
path_destination <- paste0(path_ng, "sel_images/")
```

```{r LOAD TABLES FROM ACCESS}

# package 
require(RODBC)

# seznam namigrovanych invc

dbname <- paste0("M:/03 klienti/narodni galerie - NG/ng konverze/2024-CKS-plastiky+Nm/NGP-P+Nm.accdb") 
con <- RODBC::odbcConnectAccess2007(dbname)
RODBC::sqlTables(con)
sbirka <- RODBC::sqlFetch(con, "Sb_data") %>% select(invc, MINVC)

# cesta k obrazkum

dbname <- paste0("M:/03 klienti/narodni galerie - NG/ng zdroj dat/2024-CKS-plastiky+Nm/zdroj_Ostatni.accdb") 
con <- RODBC::odbcConnectAccess2007(dbname)
RODBC::sqlTables(con)
image <- RODBC::sqlFetch(con, "Vv_images") %>% select(ident_id, data_1, pozice, sb_view, popis)

# seznam obrazku na flashce

Limages <- list.files(path_img, pattern = "\\.jpg", recursive = T,
                                    full.names = TRUE)

folder <- as.data.frame(Limages) %>% 
  mutate(filepath = sub("D:/Fotoarchiv NG/", "", Limages), 
         filename = sub(".*/", "", Limages)) %>% 
  rename("path" = 1)
...
dup_sbirka <- sbirka %>% 
  group_by(invc) %>% filter(n() != 1) # 0

dup_image <- image %>% 
  group_by(ident_id) %>% filter(n() != 1) # 75517

dup_folder <- folder %>% 
  group_by(filename) %>% filter(n() != 1) # 174
...
rel <- left_join(sbirka, image,  by = c("invc" = "ident_id"), keep = T) %>% 
  arrange(MINVC, pozice)

pairY <- rel %>% 
  filter(!is.na(ident_id)) %>% 
  mutate(filepath = sub("cksng/fotoarchiv/|cksng/fotodigi/", "", data_1),
         filename = sub(".*/", "", data_1))

# invc, ke kterym neni obrazek n = 9460
pairN <- rel %>% 
  filter(is.na(ident_id)) 
write.table(pairN, file = paste0(path_ng, "NG_MINVC_bez_obrazku.csv"), 
            sep = ";", quote = T, row.names = F, na = "", fileEncoding="cp1250")

obr <- pairY %>% 
  mutate(status = ifelse(filepath %in% folder$filepath, "Y", "N")) 
  # filter(status == "Y") %>% 
  # distinct()

obrY <- obr %>% filter(status == "Y") # n 8617
obrN <- obr %>% filter(status == "N") # n 1465
write.table(obrN, file = paste0(path_ng, "NG_MINVC_bez_souboru.csv"), 
            sep = ";", quote = T, row.names = F, na = "", fileEncoding="cp1250")
  
# celkem 8620 obrazku, ktere vychazi z parovani mezi vsemi 3 zdroji
...
mus_tab <- obrY %>%  
  mutate(
    '1uloha' = "PREDMET_PRIVATE",   # !! povinne !!                            
    '2cislo' = as.factor(MINVC),   # !! povinne !!     
    '3poradi' = pozice,   # !! povinne !!          
    '4nazev' = popis,                                   
    '5typDokumentuNazev' = "",          
    '6url' = sub(".*/", "", data_1),    # !! povinne !!
    '7nahled' = ifelse(pozice == 1, "1", "0"),       # priznak {0;1}                 
    '8vodoznak' = "0",      # priznak {0;1}                  
    '9tisk1' = "0",          # priznak {0;1}                     
    '10tisk2' = "0",         # priznak {0;1}                   
    '11poznamka' = ifelse(sb_view == "N", "sb_view = N", ""), 
    '12neDigitalni' = "", 
    '13umisteni' = "", 
    '14publikace' = "",
    '15literaturaNazev' = "", 
    '16literaturaSlovy' = "", 
    '17APlicenceCC' = "", 
    '18APvariantaCCOznaceni' = "", 
    '19APautorSubjektKod' = "", 
    '20APdrzitelSubjektKod' = "", 
    '21APtypLicence' = "", 
    '22APcasovaLicence' = "", 
    '23APuzemniLicence' = "", 
    '24APmnozstevniLicence' = "", 
    '25APdatumVyprseni' = "", 
    '26APpoznamka' = ""
    ) %>% 
  select(-ident_id, -data_1, -pozice, -invc, -MINVC, -sb_view, -popis, 
...
save <- paste0(path_ng, "NG_P_Nm_import_obrazky.csv")   
...
```{r}
to_copy <- obrY %>% 
  mutate(path = str_replace(data_1, "cksng/fotoarchiv","D:/Fotoarchiv NG")) %>% 
  select(path, filepath, filename)

loops <- 0

for (i in to_copy$path) {
  file.copy(i, path_destination, recursive = TRUE)
  loops <- loops + 1
   print(i)
   print(loops)
} 

cat("Number of loops (files copied):", loops, "\n")

# melo byt 8620, ve slozce je realne jen 8616 --> 4 chybi
# druhe kolo 8617

```

```{r  MISSING COP IMAGES}

Ltobe <- to_copy %>% 
  mutate(filename2 = sub(".*/", "", filename)) %>% 
  select(filename, filename2) %>% 
  group_by(filename2) %>% mutate(n = seq_along(filename2))
Lact <- as.data.frame(list.files(path_destination, pattern = "\\.jpg", recursive = T,
                                 full.names = F)) %>% 
  rename("in_folder2" = 1)  
  mutate(in_folder = str_replace(in_folder2, "", ""))

check <- left_join(Ltobe, Lact, by = c("filename" = "in_folder"), keep = T)

# 24xbuy_xxxx.jpg je ve slozkach DVAKRAT! 
# 02/31/24xbuy_xxxx.jpg a 06/09/24xbuy_xxxx.jpg
...
divny export v csv
oddelovac je |, ale excel nechce nacist -> Rko taky ne :/

```{r TRIAL} 
path_data <- "C:/Users/katerina.krizova/Downloads/"

df <- read.table(paste0(path_data, "data(in).csv"),
                 fileEncoding="cp1250", sep = "|") %>% 
  filter(grepl("uid_doc|\"id\"|\"nvg_id\"|\"create_uid\"|\"create_date\"", V1)) %>% 
  str_replace(V1, "|\"","-")
...
# HELA NG stavy 3
31.1.2025
M:\03 klienti\narodni galerie - NG\02 ng konverze\2025-CKS-Arch+F+X\proKK

```{r}
require(readxl)

Rev_data <- read_excel("M:/03 klienti/narodni galerie - NG/02 ng konverze/2025-CKS-Arch+F+X/proKK/proKK_revize.xlsx", col_types="text")
Sb_stav <- read_excel("M:/03 klienti/narodni galerie - NG/02 ng konverze/2025-CKS-Arch+F+X/proKK/proKK_stav.xlsx", col_types="text")

revdata <- Rev_data %>% 
  select(sb_data_id, rev_data_id, datum_rev) %>% 
  mutate(datum_rev = str_remove(datum_rev, " 00:00:00"),
         MDATE_rev = as.POSIXct(datum_rev)) %>% 
  group_by(sb_data_id) %>% 
  mutate(isDupl = ifelse(n() == 1, F, T)) %>%
  ungroup() %>% 
  arrange(sb_data_id, MDATE_rev) %>% 
...
sbstav <- Sb_stav %>% 
  ...
  #                           sb_data_id == "41380" ~ "2022-09-06 00:00:00",
  #                           TRUE ~ datum)) %>% 
  select(sb_data_id, datum, MSTAV, MPOPISSTAVU) %>% 
  drop_na(MSTAV) %>% 
...
write.table(fin, file = "M:/03 klienti/narodni galerie - NG/02 ng konverze/2025-CKS-Arch+F+X/proKK/KK_revize_stavy_n12905_TRIM.csv", 
            quote = F, row.names = F, 
            sep = ";", dec = ",", 
            na = "", fileEncoding="cp1250") # quote = F proto, ze v datech jsou uvozovky a v prvnim pripade pri ulozeni pridava backslash
...
=======
>>>>>>> a7c64368e1a29af04b265c72b7609886510c093f
