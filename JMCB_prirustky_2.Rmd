...
# --------------------------------------- sign DCM / DC   ***bude se vubec migrovat?***
...
```{r}
CORE_st <- readxl::read_excel(paste0(path_kk, "data/KK_red.xlsx"), sheet = "St", col_types = "text") # warning asi ok
```
...
a <- CORE_st %>% 
  ...
  mutate(Mpodsb = ifelse(podsbirka == "19 - M", "19", podsbirka), 
         ...
         POZN20_podsb = case_when(is.na(podsbirka) ~ paste0("Podsbírka č.: neuvedena"),
                                  ...
                                  TRUE ~ NA)) %>% 
  # -------------------------------------------------- prirc4 - part 1
  separate(prirc, into=c("Mporc", "Mrok") , sep="/") %>% 
  mutate(rok12 = paste0("19", Mrok),
         Mprirc = paste0(Mpref, Mporc, "/", rok12)) %>% 
  group_by(Mprirc) %>% 
  mutate(isDupl_prirc = ifelse(n() == 1, F, T),
         Msub = ifelse(isDupl_prirc == F, NA, seq_along(Mprirc))) %>% 
...
# write.table(ocr, file = paste0(path_kk, "import/", proj, "_obdobiCiselneRady_II.csv"),
#             quote = T, row.names = F, 
#             sep = ";", dec = ",", 
#             na = "", fileEncoding="UTF-8")
# ```
