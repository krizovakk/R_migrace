...
aa <- CORE_IIIc %>%
  ...
  mutate(Mpref = "III",
         ...
         typ30 = "P", 
         datzap98 = paste0("31.12.", rok12)) %>% 
  relocate(sbirka1, cisrada3, prirvyraz9, rok12, pocetks13, evid15, typ30, datzap98, .before = zdroj) %>% 
  mutate(isHead = ifelse(is.na(prirc)&!is.na(datumNabyti), "NADPIS", "ZAZNAM")) %>% 
  relocate(isHead, .after = zdroj) 
...
bb <- aa %>% 
 # -------------------------------------------------- prirc4 
  mutate(Mprirc1 = ifelse(isHead == "NADPIS", lead(prirc), prirc),
         Mprirc2 = Mprirc1) %>% # budeme doplnovat v dalsim kroku 
  relocate(Mprirc1, Mprirc2, .after = prirc) %>% 
  group_by(rok12) %>%
  fill(Mprirc2, .direction = "down") %>%
  ungroup() 
