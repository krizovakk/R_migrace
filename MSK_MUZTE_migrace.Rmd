...
path_proj <-  "C:/Users/krizova/Documents/R/MIG/MUZTE/" 
path_muzte <- 
...
bach_adr <- bach %>% 
  ...
  mutate(
    Msberatel = case_when(sberatel == "Buša E." ~ "Burša E.",
                          ...
                          sberatel == "Pavlíkovi P. A J." ~ "Pavlíkovi P. a J.",
...
bach <- bach_adr %>%
  ...
  select(-status_adresar, status_adresar) # UNCOMMENT TO COMMIT
...
bach_lok <- bach %>% 
  mutate(loknaz1 = katastr,  # ?????
         okrnaz2 = ifelse(str_detect(oblast, "^[:upper:]{2},\\s[:upper:]{2}"), 
                                     substr(oblast, 5,6), paste("")),
         statkod3 = str_extract(oblast, "^\\w\\w"),
         oblnaz4 = oblast,
         lokzem5 = case_when(statkod3 == "ČR" ~ "Česko",
                             statkod3 == "PL" ~ "Polsko",
                             statkod3 == "SK" ~ "Slovensko",
                             statkod3 == "SR" ~ "Slovensko")) #ČR 11096  PL 24 SK 17 SR 492  NA's 69
...
mus_tab <- mod_tab %>% 
  select(
    '1lokalitaNazev' = loknaz1,  # !! povinne !!
    '2okresNazev'= okrnaz2,
    '3statKod' = statkod3, 
    '4oblastNazev'= oblnaz4,
    '5lokalitaZeme' = MZEME, 
    '6lokalitaCtverec'= X,
    '7lokalitaMapa' = X, 
...
#   select(-status_lokality, status_lokality) # UNCOMMENT TO COMMIT
...
tax <- tax0 %>% 
  mutate(MDRUH = case_when(str_detect(druh, "×") ~ paste(word(druh, 1), word(druh, 2), word(druh, 3)), 
                            TRUE ~ paste(word(druh, 1), word(druh, 2))),
         MSUBSP = case_when(str_detect(druh, "subsp.") ~ paste(word(druh, 1), word(druh, 2), word(druh, 3), word(druh, 4)), 
                            TRUE ~ NA),
