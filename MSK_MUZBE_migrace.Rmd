...
biolib <- read_excel("M:/03 klienti/kraj moravskoslezsky/muzeum beskyd - MUZBE/
                     02 muzBE konverze/BIO/importDK/slovniky/muzbe_import_Plantae_Taxon.xlsx") 
...
prac_bot <- bot %>% 
  ...
  select(MINVC, celed, druh, syn, var) %>% 
  mutate(
...
bot_ohr <- prac_bot %>% select(MINVC, MNOMEN)
...
con <- RODBC::odbcConnectAccess2007(dbname)      # set the connection
rej <- RODBC::sqlFetch(con, "Rejstřík_Systém")   # read specific table
