...
modify_tab <- bot %>% 
  mutate('1sbirkaCisloEvidInt' = "MUZBE",
         '2podsbirkaCislo' = "Pb",
         MFOND = "B",
         MFONDNAZ = "Botanika", 
         MEMPTY = "")
...
mus_tab <- modify_tab %>% 
  select(
    ...
    '5poznamka' = MEMPTY, '6popisPubl'= MEMPTY,
...
sort(unique(modify_tab$MSKUPNAZ))
...
mus_tab <- modify_tab %>% 
  select(
    ...
    '6nazev'= MPODSKUPNAZ,
    '7poznamka'= MEMPTY,
    '8popisPubli' = MEMPTY, '9poradi'= MEMPTY,
    '10oaiId'= MEMPTY, '11oaiDatumHarvestingu'= MEMPTY, 
    '12oaiDatumModifikace' = MEMPTY) %>% 
  distinct()
...
modify_tab <- bot %>% 
  ...
  #oblast
  mutate(oblast = str_trim(oblast),
         Mfyt = case_when(oblast == "19 Bilé Karpaty stepní" ~ "19 Bílé Karpaty stepní",  # preklepy
                          oblast == "22 Halštrovská vrchovina" ~ "22 Halštrovská vrchovina",
                          oblast == "58 Žacléřsko" ~ "58a Žacléřsko",
                          oblast == "63i Hrebečovská vrchovina" ~ "63i Hřebečovská vrchovina",
                          oblast == "74 Opavská pahorkatina" ~ "74b Opavská pahorkatina", 
                          TRUE ~ oblast),
         MZEME = case_when(str_detect(oblast, "^Polsko") ~ "Polsko",
                           str_detect(oblast, "^Slovensko") ~ "Slovensko", 
                           str_detect(oblast, "^Andorra") ~ "Andorra", 
         ...
         MSTKOD = case_when(str_detect(oblast, "^Polsko") ~ "PL",
                           ...
                           str_detect(oblast, "^Rumunsko") ~ "RO", 
                           TRUE ~ "CZ"),
         MFYTOCH = ifelse(Mfyt %in% fy$fy_id|Mfyt %in% fy$naz, Mfyt, NA),
         # Mkat_anone = ifelse(katastr %in% ku$ku_name, "katastr", "nekatastr"), 
         # MKATASTR = ifelse(katastr %in% ku$ku_name, katastr, NA), 
  ...
  #  # 22 Halstrovska vrchovina - nechapu, proc se neparuje s fytochorionem
  # mutate(MFYTOCH = ifelse(MOBLAST == "22 Halštrovská vrchovina", "22 Halštrovská vrchovina", MFYTOCH),
  #        MOBLAST = ifelse(MOBLAST == "22 Halštrovská vrchovina", "", MOBLAST)) %>% 
...
mus_tab <- modify_tab %>% 
  select(
    ...
    '5lokalitaZeme' = MZEME, 
    '6lokalitaCtverec'= MEMPTY,
    '7lokalitaMapa' = MEMPTY, 
    '8lokalitaZemepisnaSirka'= MEMPTY,
    '9lokalitaZemepisnaDelka'= MEMPTY, 
    '10lokalitaNadmorskaVyska'= MEMPTY,
    '11lokalitaPoznamka'= MEMPTY, 
...
mus_tab <- modif_tab %>% 
  select(
    ...
    '8SPCisloDo'= MEMPTY, 
    '9SPPoradoveCisloDo'= MEMPTY, 
    '10SPPoradoveCisloSub' = MEMPTY,
    '11SPPoradoveCisloSubDo' = MEMPTY, 
    ...
    '30sirkaMinuty' = MEMPTY, '31sirkaVteriny' = DatZm_S, 
...
jpg_museion <- read_excel(paste0(path_kd, v, "/KontextovyDokument_export.xlsx")) %>% 
  select(Číslo, URL) %>% 
  mutate(MFN = sub("_original", "", URL))
